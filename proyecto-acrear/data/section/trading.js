// data/section/trading.js

export const tradingData = {
  es: {
    title: 'Catalogo de Productos',
    products: [
      // PRODUCCIONES ANIMALES
      { key: 'carnebov', label: 'PRODUCCIÓN BOVINA' },
      { key: 'carneovi', label: 'PRODUCCIÓN OVINA' },
      { key: 'carnecap', label: 'PRODUCCIÓN CAPRINA' },
      { key: 'carnecam', label: 'PRODUCCIÓN DE CAMÉLIDOS' },
      { key: 'carnepor', label: 'PRODUCCIÓN PORCINA' },
      { key: 'avicultura', label: 'AVICULTURA' },
      { key: 'ovoproducto', label: 'OVOPRODUCTOS' },
      { key: 'apicultura', label: 'APICULTURA' },
      { key: 'lana', label: 'LANA' },
      
      // FRUTAS
      { key: 'frutafina', label: 'FRUTAS FINAS' },
      { key: 'frutatropical', label: 'FRUTAS TROPICALES' },
      { key: 'citricultura', label: 'CITRICULTURA' },
      { key: 'frutacarozo', label: 'FRUTAS DE CAROZO' },
      { key: 'frutossecos', label: 'FRUTOS SECOS' },
      
      // ACEITES Y DERIVADOS
      { key: 'aceitesesenciales', label: 'ACEITES ESENCIALES' },
      { key: 'harinasaceites', label: 'HARINAS Y ACEITES' },
      
      // GRANOS Y SEMILLAS
      { key: 'granos', label: 'GRANOS' },
      { key: 'semillas', label: 'SEMILLAS' },
      { key: 'industriamaní', label: 'INDUSTRIA DEL MANÍ' },
      { key: 'alfalfa', label: 'ALFALFA' },
      
      // HORTALIZAS Y ESPECIAS
      { key: 'horticultura', label: 'HORTICULTURA' },
      { key: 'ajo', label: 'AJO' },
      { key: 'especia', label: 'HIERBAS AROMÁTICAS Y ESPECIAS' },
      { key: 'alcaparras', label: 'ALCAPARRAS' },
      { key: 'lupulo', label: 'LÚPULO' },
      
      // INDUSTRIAS ESPECIALIZADAS
      { key: 'olivicultura', label: 'OLIVICULTURA' },
      { key: 'vitivinicultura', label: 'VITIVINICULTURA' },
      { key: 'tabaco', label: 'TABACO' },
      { key: 'yerba', label: 'YERBA MATE y TÉ' },
      { key: 'azucar', label: 'CAÑA DE AZÚCAR' },
      
      // CONSERVAS Y DULCES
      { key: 'encurtidos', label: 'ENCURTIDOS' },
      { key: 'conservas', label: 'CONSERVAS' },
      { key: 'dulces', label: 'DULCES' },
      { key: 'bebidas', label: 'BEBIDAS' },
      
      // LÁCTEOS
      { key: 'lacteosquesos', label: 'LÁCTEOS Y QUESOS' },
      
      // FIBRAS Y TEXTILES
      { key: 'algodon', label: 'ALGODÓN' },
      


      // LEGUMBRES
      { key: 'legumbres', label: 'LEGUMBRES' },
      
      // PESCA
      { key: 'pesca', label: 'PRODUCTOS PESQUEROS Y MARISCOS' },
      
      // FORESTAL Y MADERA
      { key: 'produccionforestal', label: 'PRODUCCIÓN FORESTAL E INDUSTRIA MADERERA' },
      
      // ARTESANÍAS
      { key: 'artesanias', label: 'ARTESANÍAS REGIONALES' },
      
      // MINERÍA
      { key: 'mineria', label: 'MINERÍA' }
    ],
    cta: {
      text: 'Mirá nuestro catálogo completo aquí',
      btn: 'Descargar catálogo',
      href: '/catalogo-acrear.pdf'
    }
  },
  en: {
    title: 'Product Catalog',
    products: [
      // ANIMAL PRODUCTS
      { key: 'carnebov', label: 'BOVINE PRODUCTION' },
      { key: 'carneovi', label: 'OVINE PRODUCTION' },
      { key: 'carnecap', label: 'CAPRINE PRODUCTION' },
      { key: 'carnecam', label: 'CAMELID PRODUCTION' },
      { key: 'carnepor', label: 'PORCINE PRODUCTION' },
      { key: 'avicultura', label: 'POULTRY FARMING' },
      { key: 'ovoproducto', label: 'EGG PRODUCTS' },
      { key: 'apicultura', label: 'BEEKEEPING' },
      { key: 'lana', label: 'WOOL' },
      
      // FRUITS
      { key: 'frutafina', label: 'BERRIES' },
      { key: 'frutatropical', label: 'TROPICAL FRUITS' },
      { key: 'frutacarozo', label: 'FRUIT GROWING' },
      { key: 'frutossecos', label: 'NUTS' },
      { key: 'perasymanzanas', label: 'PEARS AND APPLES' },
      { key: 'cerezas', label: 'CHERRIES' },
      { key: 'citricultura', label: 'CITRUS FARMING' },
      { key: 'arandanos', label: 'BLUEBERRIES' },
      { key: 'clusterlimonero', label: 'LIME GROWING' },
      
      // OILS AND DERIVATIVES
      { key: 'aceitesesenciales', label: 'ESSENTIAL OILS' },
      { key: 'harinasaceites', label: 'FLOURS AND OILS' },
      
      // GRAINS AND SEEDS
      { key: 'granos', label: 'GRAINS' },
      { key: 'semillas', label: 'SEEDS' },
      { key: 'industriamaní', label: 'PEANUT INDUSTRY' },
      { key: 'alfalfa', label: 'ALFALFA' },
      
      // VEGETABLES AND SPICES
      { key: 'horticultura', label: 'HORTICULTURE' },
      { key: 'ajo', label: 'GARLIC' },
      { key: 'especia', label: 'AROMATIC HERBS AND SPICES' },
      { key: 'alcaparras', label: 'CAPERS' },
      { key: 'lupulo', label: 'HOPS' },
      
      // SPECIALIZED INDUSTRIES
      { key: 'olivicultura', label: 'OLIVE GROWING' },
      { key: 'vitivinicultura', label: 'VITICULTURE AND WINEMAKING' },
      { key: 'tabaco', label: 'TOBACCO' },
      { key: 'yerba', label: 'YERBA MATE' },
      { key: 'te', label: 'TEA' },
      { key: 'azucar', label: 'SUGAR CANE' },
      
      // PRESERVES AND SWEETS
      { key: 'encurtidos', label: 'PICKLES' },
      { key: 'conservas', label: 'SWEETS AND PRESERVES' },
      { key: 'dulces', label: 'SWEETS' },
      { key: 'bebidas', label: 'BEVERAGES' },
      
      // DAIRY
      { key: 'lacteosquesos', label: 'DAIRY PRODUCTS AND CHEESE' },
      
      // FIBERS AND TEXTILES
      { key: 'algodon', label: 'COTTON' },
      
      // LEGUMES
      { key: 'legumbres', label: 'LEGUMES' },
      
      // FISHING
      { key: 'pesca', label: 'FISH AND SEAFOOD PRODUCTS' },
      
      // FORESTRY AND WOOD
      { key: 'produccionforestal', label: 'FORESTRY AND WOOD INDUSTRY' },
      
      // HANDICRAFTS
      { key: 'artesanias', label: 'REGIONAL HANDICRAFTS' },
      
      // MINING
      { key: 'mineria', label: 'MINING' }
    ],
    cta: {
      text: 'View our full catalog here',
      btn: 'Download Catalog',
      href: '/catalogo-acrear.pdf'
    }
  }
  
};
